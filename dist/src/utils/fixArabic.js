...
function fixArabicText(text) {
    ...
    const reshaped = arabic_persian_reshaper_1.ArabicShaper.convertArabic(textStr);
    ...
    return reshaped.split('\n').map(line => {
        // عكس الحروف وتبديل الأقواس
        const chars = Array.from(line).reverse().map(ch => bracketMap[ch] || ch);
        const reversedLine = chars.join('');
        // إعادة ترتيب الأرقام والكلمات الإنجليزية التي تم عكسها بشكل غير مقصود (مثل 15 أو Burger)
        return reversedLine.replace(/([A-Za-z0-9\.\,\:\-\+\#\$\%\&]+)/g, (match) => {

...
exports.createCaptainOrderSchema = zod_1.z.preprocess(normalizeBranchId, zod_1.z.object({
    name: zod_1.z.string({ required_error: "Name is required" }).min(1, "Name cannot be empty").max(255),
    user_name: zod_1.z.string({ required_error: "user_name is required" }).min(1, "user_name cannot be empty").max(255),
    phone: zod_1.z.string({ required_error: "Phone is required" }).min(1, "Phone cannot be empty").max(50),
...
exports.updateCaptainOrderSchema = zod_1.z.preprocess(normalizeBranchId, zod_1.z.object({
    name: zod_1.z.string().min(1, "Name cannot be empty").max(255).optional(),
    user_name: zod_1.z.string().min(1, "user_name cannot be empty").max(255).optional(),

...
const getAllImages = async (req, res) => {
    ...
    const image = await connection_1.db.select().from(schema_1.sliders).where((0, drizzle_orm_1.eq)(schema_1.sliders.restaurantid, restaurantId));
    return (0, response_1.SuccessResponse)(res, {
...
const deleteImage = async (req, res) => {
    ...
    const image = await connection_1.db.select().from(schema_1.sliders).where((0, drizzle_orm_1.eq)(schema_1.sliders.id, id));
    if (!image[0]) {
...
const updateImage = async (req, res) => {
    ...
    const updatedUrl = await (0, handleImages_1.handleImageUpdate)(req, image[0].img, img, "sliders");
    if (!updatedUrl) {
        throw new BadRequest_1.BadRequest("Image is required.");
    }
    await connection_1.db.update(schema_1.sliders).set({
        img: updatedUrl,
        periorty: periorty,

...
export const getAllImages = async (req: Request, res: Response) => {
    ...
    const image = await db.select().from(sliders).where(eq(sliders.restaurantid, restaurantId));
    return SuccessResponse(res, {
...
export const deleteImage = async (req: Request, res: Response) => {
    ...
    const image = await db.select().from(sliders).where(eq(sliders.id, id));
    if (!image[0]) {
...
export const updateImage = async (req: Request, res: Response) => {
    ...
    const updatedUrl = await handleImageUpdate(req, image[0].img, img, "sliders");
    if (!updatedUrl) {
        throw new BadRequest("Image is required.");
    }
